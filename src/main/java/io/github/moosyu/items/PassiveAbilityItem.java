...
public interface PassiveAbilityItem {
     ...
     * @param target target that is possibly required for the ability (ongoing abilities cannot have targets)
     */
     ...
     * @param target target that is possibly required for the passive (ongoing abilities cannot have targets)
    ...
    /**
     * if isOngoing is false the effect will need to be stopped manually as soon as possible after the effect goes off (instant),
     * probably via {@link io.github.moosyu.util.UnshatteredUtils#finishInstantPassiveAbilities(ServerPlayer, LivingEntity)}}
     * @return whether isOngoing is true or not (whether the item ability is instant or needs to stay active)
     */
    boolean isOngoing();

    /**
     * @return true if the ability triggered effect needs to be rerun every single tick (assuming abilityConditionsMet is true)
     */
